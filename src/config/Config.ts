...
export class Config {
  ...
  private loadConfig(): BotConfig {
    return {
      ...
      // 监控配置
      monitorMode: this.parseMonitorMode(process.env.MONITOR_MODE || 'CATEGORY'),
  ...
  private validateConfig(): void {
    ...
    if (this.config.monitorMode === MonitorMode.CUSTOM && 
  ...
  public printSummary(): void {
    ...
    console.log(`   单笔金额范围: $${this.config.minPositionSize} - $${this.config.maxPositionSize}`);
    ...
    console.log(`   监控面板: ${this.config.enableDashboard ? '启用' : '禁用'}`);
    console.log('━'.repeat(60) + '\n');
  }
